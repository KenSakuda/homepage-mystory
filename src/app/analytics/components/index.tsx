...
export function AnalyticsOutsourceSection({
  ...
  leads?: string[];
  reasonsTitle?: string;
  reasons?: { term: string; desc: string }[];
  costTitle?: string;
  costBodies?: string[];
  faqTitle?: string;
  ...
  return (
    <section className={styles.appendSection} aria-labelledby={`${id}-title`}>
      {/* タイトル＆リード */}
      <h2 id={`${id}-title`} className={styles.h3}>
        {title}
        <span className={styles.h3Underline} />
      </h2>
      {leads.map((p, i) => (
      ...
      {reasons?.length > 0 && (
        <>
          <div className={styles.h4}>{reasonsTitle}</div>
          <div className={styles.appendCard}>
            <dl className={styles.dlList}>
              {reasons.map((r) => (
                <div className={styles.dlRow} key={r.term}>
                  <dt>{r.term}</dt>
                  <dd>{r.desc}</dd>
                </div>
              ))}
      ...
      {costTitle && (
        <>
          <div className={styles.h4}>{costTitle}</div>
          {(costBodies ?? []).map((p, i) => (
            <p key={i} className={styles.body}>
              {p}
            </p>
      ...
      {faqTitle && faqs && faqs.length > 0 && (
        <>
          <div className={styles.h4}>{faqTitle}</div>
          <div className={styles.faqWrap}>
            {faqs.map((f, i) => (
              <details className={styles.acItem} key={i}>
                <summary className={styles.acSummary}>{f.q}</summary>
