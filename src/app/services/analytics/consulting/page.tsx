    ...
    <main className={styles.page}>
      <Hero
        image={{
          ...
          alt: "データ分析委託・外注にも対応した受託分析 / データ利活用支援サービスのイメージ",
        }}
        title="受託分析 / データ利活用支援"
      />
        ...
        main={
          <>
            <LeadBlock
              ...
              body={[
                "データ分析をもとにビジネスを発展させるためには、正しい“洞察”から具体的な“アクション”へ繋ぐことが非常に重要です。社内だけでは対応しきれない分析業務を専門のデータ分析会社に委託・外注したい場合にも、MyStoryの『受託データ分析・データ利活用支援』サービスは、統計解析・機械学習の高度な技術力に、行動経済学・消費者行動理論・社会科学の専門的知見を組み合わせることで、単なる分析結果の報告にとどまらず意思決定と施策実行まで伴走します。",
                "　",
                "コンサルティングやマーケティング業務の経験が豊富なメンバーも在籍しておりますので、クライアントの課題に応じて最適なチーム体制を組成し、プロダクト戦略や経営戦略から、価格戦略、需要予測、顧客維持・育成、コミュニケーション最適化まで、データ解析・データ分析をビジネス成果に直結させる支援をご提供します。",
              ]}
            />

            <ServiceImage
              src="/data_analytics_consulting_mystory_capability.png"
              alt="MyStoryのデータ分析委託・外注におけるケイパビリティ"
            />

            <a id="sec-feature" />
            <FeatureHighlights
              title="MyStoryのデータ分析委託・外注サービスの特徴"
              items={[
                {
                  subtitle: "統計学・機械学習の確かな技術基盤",
                  lead: "欠損補完・外れ値対応などデータ前処理から、特徴量エンジニアリング、モデル作成、交差検証・汎化性能評価、SHAP等によるモデル解釈まで、機械学習や統計解析の全フローを深い専門性をもとに正確に対応可能。ABテスト/効果検証の設計も精緻に行えるので、データ分析に必須の“正確さ”と結果の“再現性”を担保し、意思決定に耐えるアウトプットを提供します。データ分析業務を部分的に委託・外注するケースにも柔軟に対応できます。",
                  image: {
                    src: "/data_analytics_consulting_feature1.png",
                    alt: "統計・機械学習の技術基盤によるデータ分析委託のイメージ",
                  },
                  links: [
                    {
                ...
                },
                {
                  ...
                  lead: "行動経済学や消費者行動理論の各種理論をもとに、仮説立案・実験デザイン・モデル作成を実施。分析結果から言えることに加え、消費者のクセや感情・認知特性を踏まえた制度設計・価格/プラン提示・コミュニケーション設計を提言できます。データ解析の委託・外注においても、単なるレポート納品ではなく施策レベルまで落とし込んだ提案を行います。",
                  image: {
                    src: "/data_analytics_consulting_feature2.png",
                    alt: "行動経済学・消費者行動理論の統合によるデータ解析のイメージ",
                  },
                },
                {
                  subtitle: "本質を捉えるコンサル力で“分析をビジネスに繋ぐ”",
                  lead: "データサイエンティストは技術偏重でビジネス視点が薄い。—そんな“よくある課題”を、コンサルティング/マーケティング経験豊富なメンバーとのチーム体制でご支援することで解決。課題定義→仮説→検証→実装の設計力により、「そもそも分析が必要か」「どこに着目すべきか」「分析結果から何をやるべきか」を明確化。KPIに紐づくアクションへ直結させ、意思決定会議での合意形成まで伴走します。データ分析会社として、委託プロジェクトの推進も一貫してサポートします。",
                  image: {
                ...
                },
                {
                  ...
                  lead: "統計学・機械学習の専門性とビジネス設計力を掛け合わせ、“運用で勝てる”形へ落とし込み。ダッシュボードやテンプレート、運用ガイド、コードなどを整備し、レビュー/伴走トレーニングで自社で再現できるよう内製化。人材育成経験豊富なメンバーが、チームのスキル定着まで責任を持って支援します。現在はデータ分析を外注しているが、将来的には内製化したい企業のご相談も多くいただいています。",
                  image: {
                    src: "/data_analytics_consulting_feature4.png",
                    alt: "育成・内製化の支援によるデータ分析体制構築のイメージ",
                  },
                },
              ]}
            />
            ...
            <BubbleNavSection
              title="分析サービス一覧"
              lead="主に、データ分析の委託・外注や内製化を含む以下4つの領域のご支援が可能です。※クリックすると該当サービスの説明箇所に遷移します。"
              items={[
                { label: "データ整備・\n分析環境構築", href: "#sec-env" },
                {
                  label: "受託分析",
            ...
            <SectionTitle title="受託分析（データ分析の委託・外注）" />
            <ServiceSection
              problemsTitle="こんな課題はありませんか？"
              problems={bullets([
                "・顧客データ活用をしていきたいが、分析手法がわからない",
                "・データから有効な分析結果を導き出したい",
                "・データ分析業務を効率化したい／一部を外部に委託したい",
                "・信頼できるデータ分析会社に分析・解析業務を委託・外注したい",
                "・社内に分析に詳しい人がいない",
              ])}
              overviewTitle="ご支援のイメージ"
              overview={[
                "保有するデータをお預かりし、データ分析業務を弊社に委託・外注いただく形で分析を実施し、レポートもしくは集計表を納品します。",
              ]}
              subEyebrow="アウトプットイメージ"
              subImage={{
                src: "/data_analytics_consulting_outsourcing.png",
                alt: "データ分析委託・外注におけるアウトプットイメージ",
              }}
              benefitsTitle="分析項目例"
              benefits={bullets([
                "貴社の課題をヒアリングさせていただき、課題解決のために最適なデータ解析手法・アプローチを提案させていただきます。分析内容詳細や費用については気軽にお問い合わせください。",
              ])}
            />
            ...
            <AnalyticsOutsourceSection
              ...
              leads={[
                ...
                "　",
                "データ分析・データ解析の外注・委託をご検討の企業様に対し、課題定義から仮説設計、モデル構築、ダッシュボード化、運用内製化まで、目的やリソースに応じた最適なスコープでご支援します。マーケティング領域ではデータ解析による顧客セグメンテーションや施策効果検証を通じて、売上・LTV向上に直結する提案を行っています。",
              ]}
              reasonsTitle="MyStoryが選ばれる理由"
              reasons={[
                {
                  term: "専門性の掛け合わせ",
                  desc: "統計学・機械学習・計量経済学といった定量分析の手法を活用し、消費者心理や行動科学の知見と統合。単に分析結果を提供するだけではなく、意思決定に結びつく示唆を導き出します。データ分析委託プロジェクトでも、専門性と実務知見の両面から支援します。",
                },
                {
                  term: "業種横断の実績",
      ...
      <a id="contact" />
      <FooterNav
        blockTitle="受託分析/データ利活用支援（データ分析の委託・外注）"
        links={[
